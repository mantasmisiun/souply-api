/**
 * One-shot import of scraped IKI data into Product + StoreProduct + Price.
 *
 * Usage:
 *   npx node --loader ts-node/esm src/scripts/importIki.ts
 *
 * Mirrors importRimi.ts but for IKI (chainId=3, source storeId=327). The
 * sheet layout differs from Rimi's in one respect: IKI has a single
 * `_category_` column (usually a leaf category name) instead of Rimi's
 * three-segment `_l1/_l2/_l3` breadcrumb. Other `_`-prefixed columns
 * (`_erpCode`, `_originalName`, `_apiBrand`) are scraper metadata and
 * are ignored.
 *
 * Category strategy:
 *   1. If `_category` contains a `/`, split and treat as a path
 *   2. Otherwise treat as a single-segment lookup (hits flat fallback
 *      in resolveCategoryByPath)
 *   3. Nepriskirta (id=688) on miss
 *
 * Product strategy (cross-chain reuse):
 *   - Trigram-blocked + Levenshtein-ratio match against ALL existing
 *     Products (Barbora + Rimi + previously-imported IKI). Threshold 0.85.
 *   - Miss → create new Product under resolved category (or 688).
 *
 * Price strategy:
 *   - Insert at IKI source store (327) with isFallback=1
 *   - After main loop, fan out to every other IKI store (chainId=3)
 *
...
/**
 * IKI-specific: parse the `_category_` column into a breadcrumb array.
 * If it contains `/`, split on it (and `>` as a secondary delimiter);
 * otherwise return as a single segment. Empty → empty array.
 */
...
async function main() {
    ...
    for (let i = 0; i < rows.length; i += CHUNK_SIZE) {
        ...
        try {
            ...
            for (const row of chunk) {
                ...
                // ── Resolve category from single `_category` cell ─────
    ...
    // ── Fanout to every other IKI store ────────────────────────────
    console.log('\nFanning out scraped prices to other IKI stores...');
    const [fanoutRes]: any = await pool.query(
        ...
        [IKI_CHAIN_ID, IKI_SOURCE_STORE_ID]
    );
